...
it('can be Name changed', async () => {
    let instance = await StarNotary.deployed(); // Making sure the Smart Contract is deployed and getting the instance.
    await instance.changeName("My new star", {from: owner}); // Calling the Smart Contract function claimStar
    let newName = await instance.starName.call(); // Getting the owner address
    assert.equal(newName, "My new star"); // Verifying if the owner address match with owner of the address
});
